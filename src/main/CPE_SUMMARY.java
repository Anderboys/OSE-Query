...
//	static String Series="B001";
	...
	public static void main(String[] args){
		boolean conector=true;
		 File archivos = new File("/home/acalcina/Escritorio/CPE_SUMMARY.txt");
...
//			((Document)  document.get("Status")).replace("Status", "VOIDED", "ACCEPTED");
	...
	public static  String findOneTicket(String identifier, MongoDatabase db) {
		...
		boolean exist=false;
		MongoCursor<Document> resulta = db.getCollection("CPE_SUMMARY").find(filter).iterator();
		...
		while (resulta.hasNext()) {
			...
			valRespuesta=PartyIdentification+"\t" +DocumentType+"\t"+Series+"-"+identifier +"\t"+IssueDate+"\t"+PayableAmount ;
